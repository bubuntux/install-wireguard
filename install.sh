...
DUDE
  ...
  if ip link add dev test type wireguard; then
    ...
    echo "**** The module is not active, review the logs. Try to install the module manually https://www.wireguard.com/install/ ****"
    exit 1
  fi
else
  echo "**** Kernel headers don't seem to be available, Try to install the module manually https://www.wireguard.com/install/ ****"
  exit 1
fi
